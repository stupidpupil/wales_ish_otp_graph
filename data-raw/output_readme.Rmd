...
The graph file was last updated at `r file.mtime("output/opentripplanner/graph.obj")  %>% strftime("%H:%M %Z on %d %B %Y", tz="Europe/London")`. (The graph should update weekly.)
...
readr::read_csv("output/test_journeys.csv") %>% mutate(
  ...
  tidyr::pivot_wider(Description, names_from=publicDescription, values_from=durationDescription) %>%
  arrange(Description) %>%
  (knitr::kable)

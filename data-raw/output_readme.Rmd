...
``r paste0(java_command(), " -jar otp.jar --load .")``{style="white-space:normal;"} Eventually it'll print "Started listener bound to [0.0.0.0:8080]" and you'll be able to access the planner at `http://localhost:8080` .
...
seconds_as_hhmm <- function(secs){
  hrs <- secs %/% (60*60)
  mins <- round((secs %% (60*60))/60)
  ifelse(is.na(secs), "N/A", sprintf("%1ih%2im",hrs, mins))
}

read_csv("output/test_journeys.csv") %>% mutate(
    publicDescription = ifelse(public, "Public", "Car"),
    durationDescription = seconds_as_hhmm(durationSeconds)) %>%
    select(Description, publicDescription, durationDescription) %>%
  pivot_wider(Description, names_from=publicDescription, values_from=durationDescription) %>%
  arrange(Description) %>%
  (knitr::kable)

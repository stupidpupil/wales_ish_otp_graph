simplify_and_buffer_regions_and_nations <- function(){
  ...
  regions <- read_sf("data-raw/regions_and_nations/Regions_(December_2020)_EN_BFE.geojson") %>%
    simplify_and_buffer()

  countries <- read_sf("data-raw/regions_and_nations/Countries_(December_2020)_UK_BFE.geojson") %>%
    simplify_and_buffer()
  ...
  countries %>% rename(code = CTRY20CD, name=CTRY20NM) %>% select(code, name) %>% 
    ...
    st_write("data-raw/regions_and_nations.geojson")
}